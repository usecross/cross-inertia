...
def test_application_form_validation(page: Page, fastapi_server: str) -> None:
    ...
    expect(page.locator("p.text-sm.text-red-500:has-text('Full name is required')")).to_be_visible()
    expect(page.locator("p.text-sm.text-red-500:has-text('valid email address is required')")).to_be_visible()
    expect(page.locator("p.text-sm.text-red-500:has-text('valid phone number is required')")).to_be_visible()
    expect(page.locator("p.text-sm.text-red-500:has-text('complete address is required')")).to_be_visible()
    expect(page.locator("p.text-sm.text-red-500:has-text('minimum 50 characters')")).to_be_visible()
...
def test_application_form_submission(page: Page, fastapi_server: str) -> None:
    ...
    assert re.match(r".*/cats/1$", page.url), f"Expected URL to match /cats/1, got {page.url}"
...
def test_form_character_counter(page: Page, fastapi_server: str) -> None:
    ...
    expect(page.locator("p.text-sm.text-muted-foreground:has-text('0 characters')")).to_be_visible()
    ...
    expect(page.locator("p.text-sm.text-muted-foreground:has-text('22 characters')")).to_be_visible()
